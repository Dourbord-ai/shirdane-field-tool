...
DECLARE
  ...
  v_sex smallint;
BEGIN
  -- Sex guard: confirmed male animals (sex <> 0) must never carry fertility cache.
  -- We intentionally do NOT treat sex IS NULL as male yet (pending data audit).
  ...
  IF v_sex <> 0 THEN
    UPDATE public.cows SET
      last_erotic_date = NULL,
      last_inoculation_date = NULL,
      last_pregnancy_date = NULL,
      last_abortion_date = NULL,
      last_birth_date = NULL,
      last_dry_date = NULL,
      last_rinse_date = NULL,
      last_clean_test_date = NULL,
      last_sync_date = NULL,
      last_fertility_status = NULL,
      last_fertility_status_date = NULL,
      is_pregnancy = NULL,
      is_dry = NULL,
      updated_at = now()
    WHERE id = p_cow_id;
...
-- One-time backfill: clear stale fertility cache on confirmed male cows
UPDATE public.cows SET
  last_erotic_date = NULL,
  last_inoculation_date = NULL,
  last_pregnancy_date = NULL,
...
WHERE sex <> 0
  AND (
    last_erotic_date IS NOT NULL OR
    last_inoculation_date IS NOT NULL OR
    last_pregnancy_date IS NOT NULL OR
    last_abortion_date IS NOT NULL OR
    last_birth_date IS NOT NULL OR
    last_dry_date IS NOT NULL OR
    last_rinse_date IS NOT NULL OR
    last_clean_test_date IS NOT NULL OR
    last_sync_date IS NOT NULL OR
    last_fertility_status IS NOT NULL OR
    last_fertility_status_date IS NOT NULL OR
    is_pregnancy IS NOT NULL OR
    is_dry IS NOT NULL
  );
