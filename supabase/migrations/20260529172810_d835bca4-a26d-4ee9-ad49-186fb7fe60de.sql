DROP FUNCTION IF EXISTS public.list_factors_filtered(timestamptz, timestamptz, text, uuid, text, text[], text[], int, int);
...
-- Backfill: any factor with a sepidar_voucher_id should be lifecycle 'posted'.
UPDATE public.factors
SET lifecycle_state = 'posted',
    last_posting_error = NULL
WHERE sepidar_voucher_id IS NOT NULL
  AND (lifecycle_state IS DISTINCT FROM 'posted');