  ...
  IF NEW.lifecycle_state IS DISTINCT FROM OLD.lifecycle_state THEN
    ...
    ELSIF NEW.lifecycle_state = 'approved' THEN
      ...
      UPDATE public.finance_payment_requests
       ...
       RETURNING id INTO v_req_ids;

      -- Also approve their items (Bug 2). Skip items already in a terminal
      -- or post-approval state, deleted, or already linked to a voucher /
      -- bank transaction.
