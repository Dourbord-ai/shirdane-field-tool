...
BEGIN
  ...
  -- Latest STATUS-bearing event only — ignore rows that lack both a status id and
  -- an operation id (e.g. raw pregnancy_check rows with only a result code), so
  -- a paired fertility_status row with the actual derived status wins.
  ...
  UPDATE public.cows SET
    ...
    -- Fresh derived status overwrites cache (no COALESCE) so stale values clear.
...
-- Backfill: rebuild fertility cache for every female cow so all views and the
-- /reports bar chart immediately reflect the corrected derivation logic.
DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT id FROM public.cows WHERE sex = 0 LOOP
    PERFORM public.rebuild_cow_fertility_cache(r.id);
  END LOOP;
END $$;