-- Phase 4: extend submit_payment_request RPC so new settlement items can
-- explicitly persist the Phase-3 lifecycle fields (payment_method,
-- settlement_subject_type, due_date, execution_status, execution_priority).
-- Legacy rows are NOT touched. The RPC only INSERTs new rows.
...
BEGIN
  ...
  -- Insert items. Phase-4 additions: payment_method / settlement_subject_type
  -- / due_date / execution_status / execution_priority are NULL-safe and only
  -- written when explicitly provided by the caller. Defaults applied here
  -- (pending / priority 3) so historical INSERT callsites that don't yet
