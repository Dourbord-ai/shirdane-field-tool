...
BEGIN
  ...
  FOR v_i IN 1..v_n LOOP
    ...
    END IF;
    IF v_factor_type_id = 2 AND v_existence_statuses[v_i] NOT IN (1,2,3,4) THEN
      RETURN jsonb_build_object('id', NULL, 'success', false, 'message', format('ردیف %s: نوع خروج نامعتبر است.', v_i));
    END IF;
  END LOOP;

  -- Canonical convention (see src/lib/cowPresence.ts):
  --   existancestatus IS NULL or 0  → cow is present in herd
  --   1=sold, 2=dead, 3=slaughtered, 4=other exit
  IF v_factor_type_id = 1 THEN
    -- Purchase: reject cows already present (0/NULL) in herd.
    IF EXISTS (
      SELECT 1 FROM public.cows
      WHERE id = ANY(v_cow_ids)
        AND COALESCE(existancestatus, 0) = 0
    ) THEN
  ...
  ELSIF v_factor_type_id = 2 THEN
    -- Sale/exit: require every selected cow to be present (0/NULL) in herd.
    IF (
  ...
  -- After a successful sale factor, mark each sold cow as exited with its
  -- per-row existence value (1=sale / 2=dead / 3=slaughter / 4=other). This
  -- runs in the same transaction, so a failure rolls back automatically.
  IF v_factor_type_id = 2 THEN
    FOR v_i IN 1..v_n LOOP
      UPDATE public.cows
        SET existancestatus = COALESCE(v_existence_statuses[v_i], 1)
        WHERE id = v_cow_ids[v_i];
    END LOOP;
  END IF;
