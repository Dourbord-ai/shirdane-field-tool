...
ALTER TABLE public.factors
  ALTER COLUMN invoice_date TYPE timestamptz USING (
    CASE
      WHEN invoice_date IS NULL OR btrim(invoice_date) = '' THEN NULL
      WHEN invoice_date ~ '^\d{4}/\d{1,2}/\d{1,2}' THEN
        (public.safe_text_to_date(invoice_date)::timestamp AT TIME ZONE 'Asia/Tehran')
      WHEN invoice_date ~ ':' THEN
        (to_timestamp(invoice_date, 'FMDD/FMMM/YYYY HH24:MI:SS') AT TIME ZONE 'Asia/Tehran')
      ELSE
        (to_timestamp(invoice_date, 'FMDD/FMMM/YYYY') AT TIME ZONE 'Asia/Tehran')
    END
  ),
  ALTER COLUMN delivery_date TYPE timestamptz USING (
    CASE
      WHEN delivery_date IS NULL OR btrim(delivery_date) = '' THEN NULL
      WHEN delivery_date ~ '^\d{4}/\d{1,2}/\d{1,2}' THEN
        (public.safe_text_to_date(delivery_date)::timestamp AT TIME ZONE 'Asia/Tehran')
      WHEN delivery_date ~ ':' THEN
        (to_timestamp(delivery_date, 'FMDD/FMMM/YYYY HH24:MI:SS') AT TIME ZONE 'Asia/Tehran')
      ELSE
        (to_timestamp(delivery_date, 'FMDD/FMMM/YYYY') AT TIME ZONE 'Asia/Tehran')
    END
  ),
  ALTER COLUMN settlement_date TYPE timestamptz USING (NULL::timestamptz);

-- Update submit_cow_factor RPC: convert Jalali FactorDate → Gregorian
-- timestamptz before INSERT, while preserving the original Jalali string
-- in sync_queue.payload so the Sepidar sync worker keeps its YYYY/MM/DD
-- wire format unchanged.
CREATE OR REPLACE FUNCTION public.submit_cow_factor(p_factor jsonb, p_details jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_factor_type_id = 1 THEN
    IF EXISTS (SELECT 1 FROM public.cows WHERE id = ANY(v_cow_ids) AND COALESCE(existancestatus, 0) = 1) THEN
  ...
  -- Group C: factors.invoice_date is now timestamptz, but callers still
  -- pass FactorDate as a Jalali "YYYY/MM/DD" string (legacy Sepidar
  -- contract). Convert here using safe_text_to_date (handles both Jalali
  -- and Gregorian inputs), then anchor at Tehran wall-clock midnight.
  v_invoice_date := CASE
    WHEN p_factor->>'FactorDate' IS NULL OR btrim(p_factor->>'FactorDate') = '' THEN NULL
    ELSE (public.safe_text_to_date(p_factor->>'FactorDate')::timestamp AT TIME ZONE 'Asia/Tehran')
  END;
  ...
  FOR v_i IN 1..v_n LOOP
    INSERT INTO public.cow_factor_details (
      ...
      v_factor_id, v_cow_ids[v_i], v_weights[v_i], v_unit_prices[v_i], v_row_prices[v_i],
  ...
  -- Keep original Jalali FactorDate in payload for Sepidar sync worker.
  ...
  RETURN jsonb_build_object('id', v_factor_id, 'success', true, 'message', 'فاکتور با موفقیت ثبت شد.');
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object('id', NULL, 'success', false, 'message', 'خطای پایگاه داده: ' || SQLERRM);
END;
$function$;
