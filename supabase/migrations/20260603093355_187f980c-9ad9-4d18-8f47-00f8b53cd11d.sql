-- Phase 5: persist per-item method-specific fields into the existing
-- `details` jsonb column. We only INSERT new rows. Legacy rows untouched.
...
BEGIN
  ...
  SELECT
    ...
    -- Phase 5: details payload. We accept either a real jsonb object or
    -- nothing at all; missing/empty values fall back to '{}' so the column
    -- is never NULL for new rows (queries can rely on `details->>'foo'`).
