...
export default function Invoices() {
  ...
  // -------------------------------------------------------------------------
  // Phase 4 — deep-link auto-focus.
  //
  // The settlement-requests list (PaymentRequestsTab) renders a Link2 button
  // per item that points here as `/invoices?focus=<factorId>`. We do NOT add
  // a new route or detail page — instead we read the query string after the
  // factors RPC has finished, find the matching row in the current page,
  // and trigger the existing `handleSelect` flow so the accordion opens
  // exactly as if the user had clicked the row themselves.
  //
  // Guards:
  //   - `focusHandledRef` ensures we run the focus logic ONCE per `focus`
  //     value, otherwise re-renders of the factors list would keep
  //     re-toggling the accordion.
  //   - If the factor isn't in the current page (server-side pagination —
  //     the operator may need to scroll/filter to find it), we surface a
  //     toast in Persian instead of silently doing nothing.
  //   - We strip the `focus` param from the URL after handling so a manual
  //     refresh doesn't keep re-opening the same factor.
  // -------------------------------------------------------------------------
  const focusHandledRef = useRef<string | null>(null);
  useEffect(() => {
    const focusId = searchParams.get("focus");
    // Bail early: nothing to focus, OR we already handled this exact id.
    if (!focusId || focusHandledRef.current === focusId) return;
    // Wait until the first factors fetch resolves so we know whether the
    // focused factor is already on the current page or not.
    if (loading) return;

    // Mark BEFORE any async work so a re-render during fetch doesn't
    // re-enter this branch (prevents infinite loops + duplicate prepends).
    focusHandledRef.current = focusId;

    // Helper to wipe the `?focus=` query after we're done so a page refresh
    // doesn't keep re-opening the same factor. `replace: true` avoids
    ...
    // ---- Fast path: factor already on the current page --------------------
    // No extra DB round-trip — just expand the existing accordion row using
    // the standard click handler so every side-effect (detail fetch, items
    // hydration, scroll behaviour) stays identical to a normal user click.
    const exists = factors.some((f) => f.id === focusId);
    if (exists) {
    ...
    // ---- Slow path: focused factor is OUTSIDE the current 50-row page -----
    // The list RPC has no `p_factor_id` filter, so we bypass it and hit the
    // `factors` table directly by primary key (RLS still applies). This is
    // the exact same select shape that `handleSelect` uses internally, so
    // the merged row is shape-compatible with FactorRow downstream.
    (async () => {
      try {
        const { data, error } = await supabase
          .from("factors")
        ...
        if (error) throw error;
        if (!data) {
          // The DB has no such row (deleted / wrong id / RLS hides it).
          // Surface a clear Persian message — this is the ONLY remaining
          // path where we still toast, because there is genuinely nothing
          // to display.
          toast.error("فاکتور موردنظر یافت نشد یا حذف شده است.");
          clearFocusParam();
          return;
        }

        // Reconstruct the same `party_name` derived field that the RPC
        // produces for normal list rows, so the accordion header renders
        // consistently. Same logic as in `handleSelect`.
        ...
        const focusedFactor = { ...(rest as unknown as FactorRow), party_name };

        // Prepend to the list — but only if it's still not present (defensive
        // against a parallel re-fetch that may have added it between the
        // `exists` check above and this point).
        setFactors((prev) =>
          prev.some((f) => f.id === focusId) ? prev : [focusedFactor, ...prev],
        );

        // Now run the standard expand flow. `handleSelect` will also re-fetch
        // the full row and merge it again (cheap, single primary-key lookup)
        // and crucially will load the product-type-specific item rows.
        handleSelect(focusId);
        clearFocusParam();
      } catch (e) {
        // Network / RLS failures land here. Toast and clean up the param so
        // the user can retry without reloading the page.
        // eslint-disable-next-line no-console
        console.error("[invoices] focus fetch failed", e);
        toast.error("خطا در بارگذاری فاکتور هدف.");
        clearFocusParam();
      }
    })();
    // We intentionally depend on `factors` and `loading` so the effect re-runs
    // when the list finishes loading or pagination changes. `handleSelect`
    // and `setSearchParams` are stable enough that omitting them from deps
    // doesn't cause issues here.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [factors, loading, searchParams]);
