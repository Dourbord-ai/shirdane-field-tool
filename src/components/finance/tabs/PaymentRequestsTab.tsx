...
function getPROrigin(row: {
  ...
  // No factor link at all → operator created the item by hand in PRDialog.
  if (!row.item_source_factor_id) return "manual";
  // Linked to a factor but NOT to a specific related-cost row → the item
  // represents the main payable line of the invoice (the seller payment).
  if (!row.item_source_related_cost_id) return "invoice_main";
  // Linked to both → the item was generated from a related-cost row
  // (freight / unloading / commission / …).
          ...
          return (
            ...
            >
              ...
              {/* Invoice-dependency badge stays parent-scoped — every item of
                  ...
                  invoice in a new tab. We prefer the item-level
                  `item_source_factor_id` when available (more precise — it
                  matches the exact item, not just the parent request), and
                  fall back to the parent-scoped invoiceLinks map. */}
              {(it.item_source_factor_id || invoiceLinks.has(r.id)) && (
                <div className="flex items-center gap-1">
                  ...
                  {/* Direct invoice navigation (Phase 4).
                      Why an <a> and not a nested <button>: the entire card is
                      already a <button>, so a nested button would be invalid
                      HTML and React DevTools would warn. An <a> avoids the
                      nested-interactive problem and gives us proper
                      target="_blank" semantics (middle-click, ctrl+click).
                      stopPropagation prevents the parent card's onClick from
                      firing (which would open PRDetail). The route is the
                      project's canonical /invoices page; the `focus` query
                      string is read by Invoices.tsx to auto-expand the
                      matching accordion row. We deliberately do NOT create
                      a new route here. */}
                  {it.item_source_factor_id && (
                    <a
                      href={`/invoices?focus=${encodeURIComponent(it.item_source_factor_id)}`}
                      target="_blank"
